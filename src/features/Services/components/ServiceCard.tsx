...
function ServiceCardComponent({
  ...
  const handleToggleFavorite = () => onToggleFavorite(service.id);
  const handleStart = () => onStart?.(service);
  ...
  const feeContent =
    service.fee === 0 ? (
      ...
      <AppCurrencyAmount amount={service.fee} locale={i18n.language} />
    );

  const visibleLocalizedTags = localizedService.tags.slice(
    0,
    visibleTags.length,
  );

  const visibleTagItems = visibleTags.map((tag, index) => ({
    tag,
    localizedTag: visibleLocalizedTags[index] ?? tag,
  }));
      ...
      <article>
        <div className="flex h-full flex-col">
          ...
          <div className="mt-4 flex min-h-7 flex-wrap gap-2">
            {visibleTagItems.map(({ tag, localizedTag }) => (
              <button
                key={tag}
                type="button"
          ...
          <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-slate-200 pt-4 text-sm">
            ...
            <div>
              ...
              <dd className="mt-1 text-aeblack-800">{feeContent}</dd>
            </div>
          </dl>

          <div className="mt-auto grid gap-2 pt-5 sm:grid-cols-2">
            <AppButton onClick={handleStart}>{t("start")}</AppButton>
